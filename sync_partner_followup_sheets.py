...
def fetch_existing_manual_entries(ssid):
    """
    Reads existing sheet data to preserve user-entered 'Last Touch' and 'Link' values.
    Returns dict: {workload_name_or_id: {"last_touch": val, "link": val}}
    """
    manual_entries = {}
    try:
        res = subprocess.run([GSHEETS, "readonly", "read", ssid, "'Follow_up'!A1:Z2000", "--json"], capture_output=True, text=True)
        if res.returncode == 0 and res.stdout.strip():
            raw_data = json.loads(res.stdout)
            if raw_data and len(raw_data) > 1:
                header_row = [str(h).strip().lower() for h in raw_data[0]]
                wkl_idx = -1
                last_touch_idx = -1
                link_idx = -1
                ...
                for idx, h in enumerate(header_row):
                    if "workload" in h:
                        wkl_idx = idx
                    elif "last touch" in h:
                        last_touch_idx = idx
                    elif h == "link" or "link" in h:
                        link_idx = idx
                
                if wkl_idx != -1:
                    for row in raw_data[1:]:
                        if len(row) > wkl_idx:
                            wkl_val = str(row[wkl_idx]).strip()
                            # Clean hyperlink formula if present to get display text or ID
                            clean_key = wkl_val
                            if '","' in clean_key:
                                clean_key = clean_key.split('","')[-1].rstrip('")')
                            
                            lt_val = str(row[last_touch_idx]).strip() if (last_touch_idx != -1 and len(row) > last_touch_idx) else ""
                            lk_val = str(row[link_idx]).strip() if (link_idx != -1 and len(row) > link_idx) else ""
                            
                            if lt_val or lk_val:
                                manual_entries[wkl_val] = {"last_touch": lt_val, "link": lk_val}
                                manual_entries[clean_key] = {"last_touch": lt_val, "link": lk_val}
    except Exception as e:
        print(f"Warning: Could not read existing manual entries from {ssid}: {e}")
    
    return manual_entries

os.makedirs("followup_data_synced", exist_ok=True)
summary_results = []
...
for cfg in PARTNER_CONFIGS:
    ...
    print(f"\n==========================================")
    print(f"Syncing Partner Tracker: {pname}")
    print(f"==========================================")
    
    # 1. Fetch existing manual edits
    existing_manual = fetch_existing_manual_entries(ssid)
    if existing_manual:
        print(f"Found {len(existing_manual)} existing manual entries to preserve.")
    
    # 2. Query latest BQ data
    sql = f"""
    SELECT 
      w.workload_id,
      w.workload_name,
    ...
    print(f"Found {len(rows)} uncommitted Tier 2 & 3 workloads.")
    ...
    for r in rows:
        ...
        # 2. Account Tier (Column C: 2 or 3)
        ...
        # 13. Last Touch (Column N) - Preserved from manual entries
        # 14. Link (Column O) - Preserved from manual entries
        preserved = (
            existing_manual.get(workload_id) 
            or existing_manual.get(workload_name) 
            or existing_manual.get(wkl_linked) 
            or existing_manual.get(wkl_display) 
            or {}
        )
        last_touch = preserved.get("last_touch", "")
        link_val = preserved.get("link", "")
    ...
    # Highlight manual input header columns (Last Touch & Link) in light green tint #E6F4EA
    subprocess.run([
        GSHEETS, "mutate", "format", ssid,
        "--sheet-id", "0",
        "--start-row", "0", "--end-row", "1",
    ...
    subprocess.run([GSHEETS, "mutate", "autosize", ssid, "--sheet-id", "0", "--start-col", "0", "--end-col", "15"], capture_output=True)
...
print("\n\n==========================================")
print("ALL 9 PARTNER FOLLOWUP SHEETS UPDATED WITH LAST TOUCH & LINK (AUTO-PRESERVED)!")
print("==========================================")
print(json.dumps(summary_results, indent=2))
with open("followup_summary_synced.json", "w") as f:
