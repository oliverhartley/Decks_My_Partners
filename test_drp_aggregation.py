...
SELECT 
  ...
  COALESCE(parent_pillar, 'Unassigned') as pillar,
  COALESCE(sales_play, 'Unassigned') as solution,
  COALESCE(scored_product, 'All Products') as product,
  COUNT(DISTINCT IF(tier_category = 'Tier 1', profile_id, NULL)) as tier1_count,
  COUNT(DISTINCT IF(tier_category = 'Tier 2', profile_id, NULL)) as tier2_count,
  COUNT(DISTINCT IF(tier_category IN ('Tier 1', 'Tier 2'), profile_id, NULL)) as total_tier1_tier2
FROM `concord-prod.service_partnercoe_general.view_delivery_capacity_dri_profile`
WHERE consolidated_partner_id IN (
...
for r in rows[:25]:
    ...
    t2 = r["f"][6].get("v")
    tot = r["f"][7].get("v")
    print(f"[{pname}] {pillar} > {sol} > {prod} | Tier 1: {t1} | Tier 2: {t2} | Total: {tot}")
