...
os.makedirs("followup_data_strict", exist_ok=True)
summary_results = []
...
for cfg in PARTNER_CONFIGS:
    ...
    for r in rows:
        ...
        row_partner_id = cells[14].get("v") or default_pid
        row_partner_name = cells[15].get("v") or pname
        ...
        # 2. Workload Name Hyperlink (Column C)
        ...
        # 3. Opportunity Name Hyperlink (Column D)
        ...
        # 4. Expert Requests (Column E)
        er_linked = parse_expert_requests(er_raw)
        
        # 11. ARR formatting (Column L)
        try:
            arr_float = float(arr)
    ...
    csv_file = os.path.join("followup_data_strict", f"{safe_name}_strict.csv")
    ...
    subprocess.run([GSHEETS, "mutate", "autosize", ssid, "--sheet-id", "0", "--start-col", "0", "--end-col", "12"], capture_output=True)
...
print("\n\n==========================================")
print("ALL 9 PARTNER FOLLOWUP SHEETS UPDATED (STRICT PARTNER ISOLATION)!")
print("==========================================")
print(json.dumps(summary_results, indent=2))
with open("followup_summary_strict.json", "w") as f:
